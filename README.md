...
- **[Mount namespace](https://man7.org/linux/man-pages/man7/mount_namespaces.7.html)** (Linux) — on startup, Hermit creates a new user, mount, and network namespace and pivots to a unique-per-run tmpfs root (created via `mkdtemp`). The network namespace provides an empty network stack with no interfaces. The tmpfs is mounted with `MS_NOSUID|MS_NODEV|MS_NOEXEC` and remounted read-only after setup completes. All capabilities are dropped from the bounding and ambient sets after namespace setup. During V8 initialization only `/proc/self/maps`, `/proc/self/status`, `/sys/devices/system/cpu`, and `/dev/urandom` are visible. After warmup, all bind-mounts are unmounted — the filesystem is completely empty (Cloudflare model). No `/proc`, `/sys`, or `/dev` remain. This prevents a post-escape attacker from reading `/proc/self/maps` (ASLR defeat) or `/proc/self/status` (host info leak). The host filesystem (source trees, secrets, `/etc`) is never reachable. In `--strict` mode (the Linux default), failure to create the namespace is fatal. In `--permissive` mode, Hermit warns and continues without filesystem isolation.
- **[Seccomp](https://man7.org/linux/man-pages/man2/seccomp.2.html)** (Linux) — a two-stage syscall filter that restricts what the process can do at the kernel level, even if the V8 sandbox is escaped. Stage 1 is installed after V8 initialization and blocks networking, exec, fork, timing, and most filesystem operations (violations trigger SIGSYS for debugging). A warmup eval then triggers V8's lazy initialization. Stage 2 is applied with `SECCOMP_FILTER_FLAG_TSYNC` to cover all threads (including V8 GC threads spawned during warmup), uses `SECCOMP_RET_KILL_PROCESS` for violations (no signal handler race window), locks down further, and blocks the `seccomp` syscall itself — preventing a post-escape attacker from loosening the filter from any thread. Argument-level filtering restricts `clone` to require `CLONE_THREAD` (no fork/namespace escape), `mmap` to `MAP_PRIVATE` only (no shared memory), `mremap` to block `MREMAP_FIXED` (no remapping over arbitrary targets), `mprotect` to block `PROT_EXEC` in jitless mode (no executable pages), `madvise` to safe flags, `futex` to safe operations (PI variants blocked — repeated source of kernel CVEs), and `openat` to read-only. `clone3` is forced to return `ENOSYS` via a stacked filter so glibc falls back to the filterable `clone` syscall.
...
- **prctl hardening** — `PR_SET_DUMPABLE` is disabled (no core dumps or ptrace attachment), `PR_SET_NO_NEW_PRIVS` is set (no privilege escalation via execve), and in jitless mode `PR_SET_MDWE` enforces kernel-level W^X (no writable+executable pages; Linux 6.3+, best-effort on older kernels).
